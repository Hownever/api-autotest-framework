...
import platform
import subprocess
from apigw_auth import gen_ompopenapi_header
...
def get_name(name):
    return name + str(time.time()).replace(".", "") + str(random.randint(1, 100))
...
def update_apigateway_sign(request, ak, sk):
    """ sleep n seconds
    """
    if ak == "USERAK":
        return request
    else:
        user_headers_dict = request["headers"]
        request["headers"] = gen_ompopenapi_header(ak, sk, request["url"], str(request.get("json", None)), request["method"], user_headers_dict)
        # request["headers"].update(user_headers_dict)
        return request


def get_revisionId(groupId, revision):
    return groupId + "_" + revision


def add_host(hosts, gwIp):
    if 'windows' in platform.platform().lower():
        if gwIp == "online":
            return
        if gwIp == "gray":
            return
        with open(r'C:\Windows\System32\drivers\etc\hosts', "a") as file_host:
            file_host.write("\n{gwIp}  {hosts}".format(gwIp=gwIp, hosts=hosts))
    else:
        if gwIp == "online":
            return
        if gwIp == "gray":
            return
        os.system('echo "{gwIp} {hosts}\n" >> /etc/hosts'.format(gwIp=gwIp, hosts=hosts))


def change_host(hosts, gwIp):
    if gwIp == "online":
        return hosts
    if gwIp == "gray":
        return hosts.replace(".jdcloud-api", "-gray.jdcloud-api")
    return hosts + ":8000"
    # elif gwIp == "gray":
    #     return hosts.replace(".jdcloud-api", "-gray.jdcloud-api")
    # elif gwIp == "online":
...
def get_value_from_str(all_str, aplit_str):
    return str(all_str).split(str(aplit_str))[1].replace("/", "")
...
def get_value_from_userinfo(userinfo, key):
    if key == "header":
        ...
        if header is not None:
            return header
        if pin is not None:
            return {"Content-Type": "application/json;charset=UTF-8", "x-jdcloud-pin": pin}
        else:
            return {"Content-Type": "application/json;charset=UTF-8"}
    return userinfo[key]


def string_contains_1(obj, value):
    if value in str(obj):
        return 1
    else:
        return 0
...
def curl(request):
    ...
    url = request.get("url", "NO_URL_FOUND")
    headers = request.get("headers", {})
    body = request.get("json", None)
    timeout = request.get("timeout", 120)
    cmd = 'curl -v -X {} {} -m {} '.format(method, url, timeout)
    for k, v in headers.items():
        cmd += ' -H "{}:{}"'.format(k, v)
    if body:
        cmd += ' -d "{}"'.format(body)
    print("==> Run curl cmd: {}".format(cmd))
    rlt = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    return rlt.communicate()
