...
def all_true(*args):
    print("==> all_true: {}: {}".format(args, all(args)))
    return all(args)


def any_true(*args):
    print("==> any_true: {}: {}".format(args, any(args)))
    return any(args)


def any_false(*args):
    print("==> any_false: {}: {}".format(args, not all(args)))
    return not all(args)


def all_false(*args):
    print("==> all_false: {}: {}".format(args, not any(args)))
    return not any(args)
...
def get_istio_state_from_response(response):
    if response.json and isinstance(response.json, dict):
        result = response.json.get("result", None)
        if result is None:
            raise exceptions.ParamsError(f"NO Result in respone: {response.json}")
        state = result.get("states", "NO_STATE")
        if state == "NO_STATE":
            raise exceptions.ParamsError(f"NO STATE in respone: {response.json}")
        elif state is None:
            return "uninstalled"
        elif isinstance(state, list) and isinstance(state[0], dict):
            status = state[0].get("status", "NO_STATUS")
            if status == "NO_STATUS":
                raise exceptions.ParamsError(f"NO STATUS in respone: {response.json}")
            return status
        raise exceptions.ParamsError(f"STATE FORMAT ERROR: {state}")
    raise exceptions.ParamsError(f"Response formate error: {response.json}")
