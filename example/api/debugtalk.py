...
def gen_JD_ompopenapi_header(request):
...
def update_JD_headers(request, base_url=None):
    if base_url is not None:
        request["url"] = base_url + request["url"]
    header = gen_JD_ompopenapi_header(request)
    for k, v in header.items():
        request['headers'][k] = v


def get_jdsf_config_version(version):
    with open("version", 'r') as fd:
        version = fd.readlines()[0].strip()
    v = version.split(".")
    ...
    try:
        ...
        v[1] = 0
        v[2] = 0
    rversion = ".".join(str(i) for i in v)
